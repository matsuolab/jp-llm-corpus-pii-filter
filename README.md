# 要配慮個人情報フィルター
- CommonCrawl記事などを`text`属性に格納したjsonlに対して，要配慮個人情報に該当する記事かどうかを判定しフィルターする
- フィルタ処理のパイプラインはhojicharライブラリを利用
- 現状の要配慮個人情報フィルタの構成. 以下の2点を満たすかで判定する．
    - 1. Rulebase: fullnameが存在し，かつ NGワードが存在する
    - 2. Classifier: PPI判定機械学習分類器による判定
